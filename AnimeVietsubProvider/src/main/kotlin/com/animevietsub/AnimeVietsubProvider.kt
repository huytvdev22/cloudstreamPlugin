...
class AnimeVietsubProvider : MainAPI() {
    ...
    companion object {
        ...
        // Danh sách các domain stream bên thứ 3 bị chết, drop kết nối hoặc bị chặn tại VN gây treo 60s
    ...
    /**
     * OkHttp client riêng cho AnimeVietsub với timeout ngắn (5s) chống treo socket 60s.
     */
    ...
    private suspend fun fetchHtml(url: String, domain: String): String {
        ...
        if (cookieMap.isEmpty()) {
            try {
                val initReq = okhttp3.Request.Builder()
                    ...
                    .build()
                val initResp = fastClient.newCall(initReq).execute()
        ...
        return try {
            ...
            val resp = fastClient.newCall(reqBuilder.build()).execute()
            ...
            if (resp.code == 403) {
                ...
                val retryReq = okhttp3.Request.Builder()
                    ...
                    .build()
                val retryResp = fastClient.newCall(retryReq).execute()
